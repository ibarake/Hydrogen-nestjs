...
import {useLoaderData, Await, useNavigate, useFetcher} from '@remix-run/react';
import {
  getSeoMeta,
  Money,
  ShopPayButton,
...
export async function loader({params, request, context}: LoaderFunctionArgs) {
  ...
  const wishes = await wishProducts.json() as Favorite[];
...
function FavoriteButton({children}: {children: React.ReactNode}) {
  const {product, wishes} = useLoaderData<typeof loader>();
  ...
  const isFavorite = wishes.find((favorite) => favorite.productId === productId.split('/').pop());
    return (
      ...
      <Show>
         ...
         * @return {JSX.Element}
         */}
        <Show.When isTrue={isFavorite ? true : false}>
          <submitter.Form id="wishlist-remove" className='wishlist-btn' method="post" action="/pages/wishlist">
            <input type="hidden" name="productId" value={productId} />
            <input type="hidden" name="operation" value={"remove"} />
            <button type="submit" className="rounded font-medium text-center py-3 px-6 bg-primary text-contrast w-full">Remove favorite</button>
          </submitter.Form>
        </Show.When>
        {/**
         * Already added button to delete favorite product
         ...
         */}
        <Show.Else>
          <submitter.Form id="wishlist-add" className='wishlist-btn' method="post" action="/pages/wishlist">
            <input type="hidden" name="productId" value={productId} />
            <input type="hidden" name="operation" value={"add"} />
            <button type="submit" className="rounded font-medium text-center py-3 px-6 bg-primary text-contrast w-full">{children}</button>
          </submitter.Form>
        </Show.Else>
      </Show>
      </> 
